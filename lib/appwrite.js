...
export const likeOrUnlike = async (userId, postId) => {
    console.log('userId', userId, 'postId', postId)
    try {
        const likedPost = await getLikedPost(userId, postId);

        if (likedPost.length > 0) {
            console.log('unliking ongoing')
            return await unlikePost(likedPost[0].$id,postId);
        } else {
            console.log('liking')
...
async function likePost(userId, postId) {
    ...
    await updateLikesCount(postId, 1);

    return like;
}

async function unlikePost(likeId, postId) {
    console.log('unliking post', postId)
    console.log('likeId', likeId)
    const like = await databases.deleteDocument(
        databaseId,
        likesCollectionId,
        likeId
    );
...
async function updateLikesCount(videoId, value) {
    try {
        // Fetch the current number of likes for the video
        const video = await databases.getDocument(databaseId, videoCollectionId, videoId);

        // Calculate the new number of likes, ensuring it doesn't go below zero
        const updatedLikesCount = Math.max((video.number_of_likes || 0) + value, 0);
        ...
        await databases.updateDocument(
            ...
            videoCollectionId,
            videoId,
            { number_of_likes: updatedLikesCount }
        );

        console.log(`Updated number_of_likes to ${updatedLikesCount}`);
        return updatedLikesCount;
    } catch (error) {
        console.error('Error updating number_of_likes:', error);
    }
}
